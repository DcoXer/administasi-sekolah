...
<aside :class="open ? 'translate-x-0 w-64' : '-translate-x-48 w-20'"
    ...
    <nav class="p-4 space-y-3 font-medium text-gray-700">
        ...
        <a href="{{ route('users.index') }}"
            class="flex items-center p-3 rounded-lg hover:bg-blue-50 
                       shadow-sm transition-all duration-300 hover:scale-[1.02]">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 text-sky-500">
        ...
        <div x-data="{ keuanganOpen: false }">
            <button @click="keuanganOpen = !keuanganOpen"
                class="flex items-center w-full p-3 rounded-lg hover:bg-green-100 shadow-sm transition-all duration-300 hover:scale-[1.02]">
            ...
            <div x-show="keuanganOpen" x-transition.scale.opacity class="ml-10 space-y-1">
                <a href="{{ route('daftar-ulang.index') }}" class="block py-2 text-sm hover:text-indigo-700">Pembayaran Daftar Ulang</a>
                <a href="{{ route('pembayaran-spp.index') }}" class="block py-2 text-sm hover:text-indigo-700">Pembayaran SPP Bulanan</a>
            </div>
        </div>
        @endif
