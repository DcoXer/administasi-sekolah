        ...
        <div class="bg-white rounded-2xl shadow-lg p-10 border relative flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between relative z-10">
                ...
                <div class="text-right">
                    <h2 class="text-sm font-semibold uppercase">Bukti Pembayaran SPP</h2>
            ...
            <div class="flex justify-between items-start gap-10 relative z-10">
                <div class="grid grid-cols-2 gap-x-10 gap-y-2 text-xs w-2/3">
                    ...
                    <div>: {{ $buktiSpp->siswa->nama }}</div>
                    <div><strong>NISN</strong></div>
                    <div>: {{ $buktiSpp->siswa->nisn }}</div>
                    <div><strong>Kelas</strong></div>
                    <div>: {{ $buktiSpp->siswa->kelas }}</div>
                    <div><strong>Bulan/Tahun</strong></div>
                    <div>: {{ $buktiSpp->bulan }} / {{ $buktiSpp->tahun }}</div>
                    <div><strong>Jumlah</strong></div>
                    <div>: Rp {{ number_format($buktiSpp->jumlah, 0, ',', '.') }}</div>
                    <div><strong>Status</strong></div>
                    <div>:
                        <span class="px-2 py-1 rounded text-white {{ $buktiSpp->status == 'sudah' ? 'bg-green-600' : 'bg-red-600' }}">
                            {{ ucfirst($buktiSpp->status) }}
                        </span>
                    </div>
                    <div><strong>Tanggal Bayar</strong></div>
                    <div>: {{ \Carbon\Carbon::parse($buktiSpp->tanggal_bayar)->format('d M Y') }}</div>
                </div>
